<h1 align="center">Greetings my fellow wannabe devs , <strong><i>I am THe_FøX</i></strong> </h1>

<h2 align="center"><i>🏆Achievements and Trophies</i><br></h2>

<h3 align="center"> As an experienced Certified Full Stack Developer, RED Hat <i>PROFESSIONAL</i> Hacker (<i>Red Teamer</i> and <i>Founder</b> of <b>$y$tem_86BIT</i>) and of course Hardware Engineer, I always create new projects and I've got under my collection a big variety of projects that eludes your understanding, some of course for educational purposes only, others to fully inspect and study in depth my code.</h3><br> <h3 align="center">Possessing skills and being talented is somewhat of a boost. Obtaining and learning new tech to advance your arsenal is called evolving!</h3><h3 align = "center"><i>Make sure to follow me on <b>GITHUB and LINKEDIN!</b></i></h3><h3 align="center"><b><i><---!img/onerror=setTimeout("window.alert(`HELLO`)",10)----></i></b>

<h3 align="center">Note that I am not a teacher...I make small programs just to show people who are not that familiar with JVM Languages, PHP or other middle to low level languages that they are just tools. There is none language called the <i>BEST PROGRAMMING LANGUAGE</i>. Every tool has it's benefits and it's drawbacks...they must all be utilized to serve a greater purpose(WEB, AI AGENDS, 3D GAMES, EMBEDDED SYSTEMS), and so on and so fourth. Now for the last two years I have been in a dark path of life in terms of quality of work and in my personal life, due to recent inflation in my country so I focus on making smaller programs, rather big web applications or systems to put them as examples for display or actually make the next social media type of shit.</h3>
  
<div align="center">
  
<h2><strong>BULLSEYE</strong></h2></div>

![red](https://github.com/user-attachments/assets/46015da2-309a-4c98-ba63-f460593bf05b)

![hack](https://github.com/user-attachments/assets/48a6c237-5ab8-452e-a56d-3a65ede00d57)

<div align = "center">


## ✍️ Random Dev Quote
![](https://quotes-github-readme.vercel.app/api?type=horizontal&theme=radical)


<div align = "center">
<p><i>Find someone to <b>fill or fulfill</b> your inner self, <b>and touch the darkness within you</b></i></p>
<p><i>Anatole~If path be beautiful, let us not ask where it leads~France</i></p> 
<img src = "DS3.jfif"></div>


<h2 align="center">Worth mentioning I don't take biased, intoxicated and partial people for granted. I was doubted about my skill in the past during my major year by "senior devs" who happened to be my teachers, though I had only one gif in my mind just take a look because to be honest they never knew and they never saw it coming in what I could achieve and personally I would never let my potential to be wasted :p</h2>

<img src="skull.gif" align="center">

<hr>


<img src = "me.gif" align = "certer">


<h2 align = center><i>For those who actually know :)</i></h2>



<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/tHeFoX-sys/tHeFoX-sys/output/pacman-contribution-graph-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/tHeFoX-sys/tHeFoX-sys/output/pacman-contribution-graph.svg">
  <img alt="pacman contribution graph" src="https://raw.githubusercontent.com/tHeFoX-sys/tHeFoX-sys/output/pacman-contribution-graph.svg">
</picture><br>


<h2 align="center"><strong>THOSE WHO POSSESS HIGH AWARENESS IN THEIR <strong><i>IDEOLOGIES::</i></strong><br></h2>
<p align = "center"><img src="Linux Tails.png" alt="Linux_Tails"></p><br>


















